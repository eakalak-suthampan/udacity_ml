...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        if testing==False:
            ...
            #self.epsilon -= numpy.exp(-1.0*self.t)
            #self.epsilon = 1 - 0.99 * math.e ** ( -math.e ** (-0.007 *(self.t - 10)))
            if self.t < 100:
                self.epsilon = 1
            else: 
                self.epsilon-=0.01
    ...
    def choose_action(self, state):
        ...
        if self.learning:
            if self.epsilon > random.random():
                ...
                maxQ_actions = [e for e in self.Q[state].keys() if self.Q[state][e] == maxQ]
...
def run():
    ...
    sim = Simulator(env, log_metrics=True, update_delay=0.000001, display=False, optimized=True)
    ...
    sim.run(n_test=10000, tolerance=0.01)
